#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdint.h>

#include <oqs/oqs.h>
#include <openssl/evp.h>
#include <openssl/rand.h>

static void print_hex(const char *label, const uint8_t *buf, size_t len) {
#if defined(DUMP_SECRETS)
    printf("%s (%zu bytes): ", label, len);
    for (size_t i = 0; i < len; i++) printf("%02x", buf[i]);
    printf("\n");
#else
    (void)label; (void)buf; (void)len;
#endif
}

static void step(const char *s) {
#if defined(TRACE_STEPS)
    static int n = 0;
    printf("\n== [%d] %s ==\n", ++n, s);
#endif
}

/* ---------------- AES-256-GCM helpers ---------------- */

static int aes256gcm_encrypt(const uint8_t *key32, const uint8_t *iv, size_t iv_len,
                             const uint8_t *pt, size_t pt_len,
                             uint8_t *ct, uint8_t tag[16]) {
    int len = 0, out = 0;
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx) return 0;
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1) goto done;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, (int)iv_len, NULL) != 1) goto done;
    if (EVP_EncryptInit_ex(ctx, NULL, NULL, key32, iv) != 1) goto done;
    if (EVP_EncryptUpdate(ctx, ct, &len, pt, (int)pt_len) != 1) goto done;
    out = len;
    if (EVP_EncryptFinal_ex(ctx, ct + out, &len) != 1) goto done; /* len==0 for GCM */
    out += len;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, tag) != 1) goto done;
    EVP_CIPHER_CTX_free(ctx);
    return out;
done:
    EVP_CIPHER_CTX_free(ctx);
    return 0;
}

static int aes256gcm_decrypt(const uint8_t *key32, const uint8_t *iv, size_t iv_len,
                             const uint8_t *ct, size_t ct_len,
                             const uint8_t tag[16],
                             uint8_t *pt_out) {
    int len = 0, out = 0;
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx) return 0;
    if (EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) != 1) goto done;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, (int)iv_len, NULL) != 1) goto done;
    if (EVP_DecryptInit_ex(ctx, NULL, NULL, key32, iv) != 1) goto done;
    if (EVP_DecryptUpdate(ctx, pt_out, &len, ct, (int)ct_len) != 1) goto done;
    out = len;
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, 16, (void*)tag) != 1) goto done;
    if (EVP_DecryptFinal_ex(ctx, pt_out + out, &len) != 1) goto done;
    out += len;
    EVP_CIPHER_CTX_free(ctx);
    return out;
done:
    EVP_CIPHER_CTX_free(ctx);
    return 0;
}

/* ---------------- Main ---------------- */

int main(int argc, char **argv) {
    const char *PLAINTEXT = (argc > 1) ? argv[1] : "hello kyber hybrid";
    size_t PT_LEN = strlen(PLAINTEXT);

    step("KEM setup (ML-KEM-768)");
    OQS_KEM *kem = OQS_KEM_new("ML-KEM-768");
    if (!kem) { fprintf(stderr, "KEM not available\n"); return 1; }
    printf("Algorithm: %s\n", kem->method_name);
    printf("pk=%zu, sk=%zu, ct=%zu, ss=%zu bytes\n",
           kem->length_public_key, kem->length_secret_key,
           kem->length_ciphertext, kem->length_shared_secret);

    step("Key generation (receiver)");
    uint8_t *pk = malloc(kem->length_public_key);
    uint8_t *sk = malloc(kem->length_secret_key);
    if (!pk || !sk) { fprintf(stderr, "alloc\n"); return 2; }
    if (OQS_KEM_keypair(kem, pk, sk) != OQS_SUCCESS) { fprintf(stderr, "keypair\n"); return 3; }
    print_hex("Receiver public key (pk)", pk, kem->length_public_key);
    print_hex("Receiver secret key (sk)", sk, kem->length_secret_key);

    step("Sender encapsulation -> shared secret + KEM ciphertext");
    uint8_t *kem_ct = malloc(kem->length_ciphertext);
    uint8_t *ss_sender = malloc(kem->length_shared_secret);
    if (!kem_ct || !ss_sender) { fprintf(stderr, "alloc\n"); return 4; }
    if (OQS_KEM_encaps(kem, kem_ct, ss_sender, pk) != OQS_SUCCESS) { fprintf(stderr, "encaps\n"); return 5; }
    print_hex("KEM ciphertext (ct)", kem_ct, kem->length_ciphertext);
    print_hex("Sender shared secret (ss_sender)", ss_sender, kem->length_shared_secret);

    step("Derive AEAD key (demo uses ss directly as AES-256 key)");
    uint8_t key[32];
    memcpy(key, ss_sender, 32);
    print_hex("AES-256 key", key, sizeof key);

    step("Encrypt plaintext with AES-256-GCM");
    uint8_t iv[12], tag[16];
    if (RAND_bytes(iv, sizeof iv) != 1) { fprintf(stderr, "RAND_bytes\n"); return 6; }
    uint8_t *ct = malloc(PT_LEN);
    if (!ct) { fprintf(stderr, "alloc\n"); return 7; }
    int ct_len = aes256gcm_encrypt(key, iv, sizeof iv,
                                   (const uint8_t*)PLAINTEXT, PT_LEN,
                                   ct, tag);
    if (ct_len <= 0) { fprintf(stderr, "encrypt fail\n"); return 8; }
    print_hex("AES-GCM IV", iv, sizeof iv);
    print_hex("AES-GCM tag", tag, sizeof tag);
    print_hex("AES-GCM ciphertext", ct, (size_t)ct_len);

    step("Receiver decapsulation -> shared secret");
    uint8_t *ss_receiver = malloc(kem->length_shared_secret);
    if (!ss_receiver) { fprintf(stderr, "alloc\n"); return 9; }
    if (OQS_KEM_decaps(kem, ss_receiver, kem_ct, sk) != OQS_SUCCESS) { fprintf(stderr, "decaps\n"); return 10; }
    print_hex("Receiver shared secret (ss_receiver)", ss_receiver, kem->length_shared_secret);

    step("Shared secret check");
    int same = memcmp(ss_sender, ss_receiver, kem->length_shared_secret) == 0;
    printf("Shared secrets equal? %s\n", same ? "yes" : "NO!");

    step("Decrypt with AES-256-GCM");
    uint8_t *pt_out = malloc(ct_len);
    if (!pt_out) { fprintf(stderr, "alloc\n"); return 11; }
    int pt_len = aes256gcm_decrypt(ss_receiver, iv, sizeof iv, ct, (size_t)ct_len, tag, pt_out);
    if (pt_len <= 0) { fprintf(stderr, "decrypt fail\n"); return 12; }
    printf("Recovered plaintext: %.*s\n", pt_len, (char*)pt_out);

    /* cleanup */
    OQS_KEM_free(kem);
    free(pk); free(sk); free(kem_ct); free(ss_sender); free(ss_receiver);
    free(ct); free(pt_out);
    return 0;
}


